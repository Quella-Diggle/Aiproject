...
window.addEventListener('error', (e) => {
  ...
  console.error('[list] window error', e.error ?? e.message);
  renderFatal(msg);
});
...
window.addEventListener('unhandledrejection', (e) => {
  ...
  console.error('[list] unhandled rejection', e.reason);
  renderFatal(msg);
});
...
try {
  ...
  app.config.errorHandler = (err, _instance, info) => {
    ...
    console.error('[list] vue error', err, info);
    renderFatal(msg);
  };
  app.use(createPinia());
  app.mount('#app');
  ...
  const detail = err instanceof Error ? err.stack || err.message : String(err);
  console.error('[list] bootstrap failed', err);
